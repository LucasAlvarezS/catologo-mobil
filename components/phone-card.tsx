    ...
    <Link href={`/telefono/${phone.id}`}>
      <Card className="group overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1 bg-card">
        <div className="relative aspect-square overflow-hidden bg-muted">
          ...
          />
          {hasPlanPrice && <Badge className="absolute top-2 right-2 bg-emerald-500 text-white">Con Plan</Badge>}
        </div>
            ...
            {hasPlanPrice ? (
              <>
                <div className="flex items-center gap-2">
                  <span className="text-lg font-bold text-emerald-600">{formatPrice(phone.precio_plan!)}</span>
                  <span className="text-xs bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded">Con plan</span>
                </div>
                {hasDiscount ? (
                  <p className="text-sm text-muted-foreground">
                    Oferta: <span className="text-primary font-medium">{formatPrice(phone.precio_descuento!)}</span>
                    <span className="line-through ml-1 text-xs">{formatPrice(phone.precio_lista)}</span>
                  </p>
                ) : (
                  <p className="text-sm text-muted-foreground">Sin plan: {formatPrice(phone.precio_lista)}</p>
                )}
              </>
            ) : hasDiscount ? (
              <div className="flex items-center gap-2">
